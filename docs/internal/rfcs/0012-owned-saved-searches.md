...
- Deliver owner-scoped identifiers, quota enforcement, isolation tests.
...
WS-0009 is an R2 production-readiness slice. G4 requires repository checks, migration probes, native unit tests, local Worker+D1 integration, query-plan checks where relevant, and production-safe smoke inspection. G5 remains human-owned and requires external runtime evidence.
...
Required evidence is stored under `evidence/WS-0009/`. Local acceptance requires `just fix && just verify`. Production acceptance additionally requires staging/production deployment evidence and independent review.
