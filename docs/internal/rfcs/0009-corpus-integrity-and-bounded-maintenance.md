# RFC 0009: Corpus integrity and bounded maintenance
...
- Deliver maintenance runs, integrity findings, bounded status repair.
...
WS-0006 is an R2 production-readiness slice. G4 requires repository checks, migration probes, native unit tests, local Worker+D1 integration, query-plan checks where relevant, and production-safe smoke inspection. G5 remains human-owned and requires external runtime evidence.
...
Required evidence is stored under `evidence/WS-0006/`. Local acceptance requires `just fix && just verify`. Production acceptance additionally requires staging/production deployment evidence and independent review.
