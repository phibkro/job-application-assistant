# RFC 0010: Versioned production read API
...
- Deliver indexed structured filters, provenance, bounded pages, OpenAPI contract.
...
WS-0007 is an R2 production-readiness slice. G4 requires repository checks, migration probes, native unit tests, local Worker+D1 integration, query-plan checks where relevant, and production-safe smoke inspection. G5 remains human-owned and requires external runtime evidence.
...
Required evidence is stored under `evidence/WS-0007/`. Local acceptance requires `just fix && just verify`. Production acceptance additionally requires staging/production deployment evidence and independent review.
