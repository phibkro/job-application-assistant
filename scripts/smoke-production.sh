...
BASE_URL="${1:?usage: smoke-production.sh <base-url> [expected-environment]}"
EXPECTED_ENVIRONMENT="${2:-production}"
...
python3 - "${OUTPUT_DIR}" "${EXPECTED_ENVIRONMENT}" <<'PY'
import json
import pathlib
import sys
...
assert health["environment"] == expected_environment, health
assert about["service"] == "job-index", about
assert about["license"] == "proprietary", about
assert about["environment"] == expected_environment, about
assert isinstance(v1_jobs["data"], list), v1_jobs

print(f"{expected_environment.capitalize()} smoke assertions passed:")
print("  health and about respond with the expected deployment shape")
print("  the public jobs feed answers")
print("  no application data was mutated")
PY
