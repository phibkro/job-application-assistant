...
describe("page", () => {
  ...
  it("requests one checkpointable feed page and nothing else", async () => {
    ...
    const client = clientOf((url) => {
      calls += 1;
      if (url.endsWith("/api/v1/feed?last=true&pageSize=5")) {
  ...
  it("tells a fresh sweep where to start, and does not re-tell a resumed one", async () => {
    ...
    expect(seenUrls).toEqual([
      "https://pam-stilling-feed.nav.no/api/v1/feed?pageSize=5",
      "https://pam-stilling-feed.nav.no/api/v1/feed/abc-123?pageSize=5",
    ]);
  });
