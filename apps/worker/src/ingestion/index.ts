 ...
 * a never-before-collected platform's sweep begins, and `Database` directly
 * for this slot's own state (`source_state`, `ingestion_runs`,
 * `ingestion_failures` — none of which `Corpus` or `SourceCatalog` know
 * about, and none of which should).
 */
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    return Ingestion.of({
      collect: makeCollect({ database, acquisition, corpus, sourceCatalog }),
    });
  }),
);
